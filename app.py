...
# ============================================================================
# 5. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ПАРСИНГА
# ============================================================================
def _norm(v):
    if pd.isna(v):
        return ""
...
# ============================================================================
# 6. УНИВЕРСАЛЬНЫЙ ПАРСЕР EXCEL-ТАБЛИЦЫ
# ============================================================================
def parse_tender_excel(file_like, sheet=0):
    ...
    Поддерживает:
      A) Отдельная строка 'ИНН' с номерами в отдельных колонках (файл 19377-ТУ).
      B) ИНН прямо в названии фирмы: ООО "..." (ИНН 1234567890) (файл 20893-ТТ).

    Суммы по лотам считаются как сумма по строкам работ/материалов между
    шапками лотов (шапка лота сама в сумму не входит).
    """
    ...
    # ======================================================================
    participants = []       # [{'inn':..., 'name':..., 'inn_col':..., 'sum_col':None}]
    participant_header_row = None

    # --- Формат B: ищем строку с "(ИНН ...)" ---
    ...
    if not participants:
        ...
        # Имена фирм — строкой выше (обычно "Наименование контрагента")
        name_row = None
        for r in range(n_rows):
            for c in range(n_cols):
    ...
    # ======================================================================
    # 2. Находим строку-шапку колонок ("Цена за ед." / "Сумма")
    # ======================================================================
    header_row = None
    for r in range(n_rows):
    ...
    # Строка "Система налогообложения" — для fallback
    tax_row = None
    for r in range(n_rows):
        for c in range(n_cols):
    ...
    # Для каждого участника находим колонку с суммой
    for p in participants:
        inn_col = p["inn_col"]
        sum_col = None

        if header_row is not None:
            # Ближайшая правая колонка с "сумма" / "стоимость всего"
        ...
        if sum_col is None:
            # Fallback: ближайшая правая из tax_cols
            for c in tax_cols:
                if c >= inn_col:
                    sum_col = c
                    break
    ...
    # ======================================================================
    # 4. Считаем суммы по каждому лоту и участнику
    # ======================================================================
    STOP_WORDS = (
    ...
    def is_stop_row(r):
        """True, если строка — служебная и не должна суммироваться."""
...
# ============================================================================
# 10. UI: ЗАГРУЗКА EXCEL-ФАЙЛА
# ============================================================================
st.markdown('<div class="search-card">', unsafe_allow_html=True)
st.markdown("### 📥 Загрузка сводной таблицы предложений")
st.markdown("Загрузите Excel-файл «Сводная оценочная таблица» или «Конкурентный лист». "
    ...
    if st.button("📊 Разобрать и заполнить", type="primary", key="parse_excel_btn"):
        try:
            ...
            for p in parsed["participants"]:
                ...
                if all(v == 0 for v in prices):
                    warnings.append(
                        f"{p['name'] or 'Без названия'} (ИНН {p['inn']}): все цены = 0"
                    )

            # --- КРИТИЧНО: чистим ключи виджетов, чтобы Streamlit подхватил
            #     новые value=... из session_state['participants']
            for k in list(st.session_state.keys()):
                if (k.startswith("inn_") or k.startswith("price_")
                        or k.startswith("del_") or k == "lots_count_input"):
                    try:
...
if int(lots_count) != st.session_state.lots_count:
    ...
    # Если пользователь вручную меняет число лотов — обновим и заголовки
    if st.session_state.lot_titles and len(st.session_state.lot_titles) != new_n:
        st.session_state.lot_titles = []
    st.session_state.analysis_result = None
    st.rerun()
...
for i, participant in enumerate(st.session_state.participants):
    ...
    with col_title:
        st.markdown(f'<div class="participant-title">Участник {i + 1}</div>',
                    unsafe_allow_html=True)
    with col_del:
        if len(st.session_state.participants) > 2:
            if st.button("✕", key=f"del_{i}", help="Удалить участника"):
                st.session_state.participants.pop(i)
                # Чистим все ключи виджетов, чтобы не «съезжали» индексы
                for k in list(st.session_state.keys()):
                    if (k.startswith("inn_") or k.startswith("price_")
    ...
    for row_start in range(0, st.session_state.lots_count, lot_cols_per_row):
        ...
        for j in range(lot_cols_per_row):
            ...
            if lot_idx >= st.session_state.lots_count:
                break
            with cols[j]:
                # Заголовок — короткий, но с полным названием в tooltip
                short_label = f"Лот {lot_idx + 1}"
                if st.session_state.lot_titles and lot_idx < len(st.session_state.lot_titles):
                    full_title = st.session_state.lot_titles[lot_idx]
                    # Обрезаем для метки
                    short_label = full_title if len(full_title) < 30 else full_title[:28] + "…"
                ...
                val = st.number_input(
                    ...
                    help=st.session_state.lot_titles[lot_idx]
                    if st.session_state.lot_titles and lot_idx < len(st.session_state.lot_titles)
                    else None
                )
...
with col_add:
    if st.button("➕ Добавить участника", use_container_width=True, key="add_participant"):
        ...
        # Удаляем ключи новых виджетов не нужно, но очистим кэш
        for k in list(st.session_state.keys()):
            if k.startswith("inn_") or k.startswith("price_") or k.startswith("del_"):
                try:
                    del st.session_state[k]
                except KeyError:
                    pass
...
with col_check:
    if st.button("🚀 Отправить на проверку", use_container_width=True,
                 type="primary", key="submit_check"):
        errors = []
...
if st.session_state.analysis_result is not None:
    ...
    for lot_idx in range(lots_count):
        ...
        if df_lot.empty:
            st.info(f"На лот «{title}» не подано ни одной заявки (все цены = 0).")
            continue
        ...
        if len(df_lot) > 2:
            with st.expander(f"Остальные участники лота «{title}»"):
